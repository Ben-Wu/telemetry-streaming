...
cat << EOF
{
    "name": "Crashes to Influx Siggen Streaming",
    "new_cluster": {
        ...
        "custom_tags": {
            ...
            "Env": "databricks-dev",
            "Type": "streaming",
            "TelemetryJobName": "crashes_to_influx"
        }
    },
    "libraries": [{"jar": "s3://net-mozaws-data-us-west-2-ops-ci-artifacts/mozilla/telemetry-streaming/crash-trace-symbolication/telemetry-streaming.jar"}],
    "email_notifications": {
    ...
    "spark_jar_task": {
        ...
        "parameters": ["--kafkaBroker","${KAFKA_BROKER}",
                       "--url","${URL}",
                       "--measurementName","crashsig",
                       "--getCrashSignature","true",
                       "--acceptedChannels","beta"]
    }
}
EOF
}

databricks jobs reset --job-id 464 --json "$(create_job_json)"